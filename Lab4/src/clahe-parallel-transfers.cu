...
double d_apply_clahe(PGM_IMG img_in, PGM_IMG *img_out) {
    ...
    chunk_h = grid_h / num_streams;

    if (chunk_h % num_streams != 0)
        chunk_h++;
    ...
    // Allocate device look-up tables
    cudaMalloc(&d_all_luts, grid_w * grid_h * 256 * sizeof(unsigned char));
    CUDA_CHECK_LAST_ERROR();
    ...
    for (int i = 0; i < num_streams; i++) {
        ...
        // Transfer image to device in chunks
        cudaMemcpyAsync(d_img_in + chunk_offset,
                        img_in.img + chunk_offset,
                        w * chunk_h * TILE_SIZE * sizeof(unsigned char),
