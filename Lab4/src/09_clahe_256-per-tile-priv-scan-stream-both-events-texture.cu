...
__global__ void render_clahe(
    ...
    const int tl = __ldg(&all_luts[(y1 * gridDim.x + x1) * 256 + val]);
    const int tr = __ldg(&all_luts[(y1 * gridDim.x + x2) * 256 + val]);
    const int bl = __ldg(&all_luts[(y2 * gridDim.x + x1) * 256 + val]);
    const int br = __ldg(&all_luts[(y2 * gridDim.x + x2) * 256 + val]);
