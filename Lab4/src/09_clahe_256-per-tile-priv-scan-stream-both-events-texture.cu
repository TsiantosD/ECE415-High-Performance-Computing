...
__global__ void render_clahe(
    ...
    int w, int h,
    const unsigned char * __restrict__ all_luts, // Changed to const restrict
    int block_offset_y,
    int full_grid_h
) {
    ...
    // Use __ldg to force Read-Only Cache (Texture Cache)
    // This significantly speeds up scattered reads from the LUT table.
    tl = __ldg(&all_luts[(y1 * gridDim.x + x1) * 256 + val]);
    tr = __ldg(&all_luts[(y1 * gridDim.x + x2) * 256 + val]);
    bl = __ldg(&all_luts[(y2 * gridDim.x + x1) * 256 + val]);
    br = __ldg(&all_luts[(y2 * gridDim.x + x2) * 256 + val]);
...
double d_apply_clahe(PGM_IMG img_in, PGM_IMG *img_out) {
    ...
    // Moved OUTSIDE the timer to prevent OS overhead from polluting the benchmark.
